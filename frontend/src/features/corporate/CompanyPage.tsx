...
function getContent<T>(contents: CompanyContent[], key: string, fallback: T): T {
...
function getTextArray(value: LocalizedArray | string[] | undefined, language: string) {
    if (!value) return [];
...
function ServiceAccordionItem({
    ...
    const footerNote = getText(item.footerNote, language);
    const tags = getTextArray(item.tags, language);
    const image = getPublicMediaUrl(item.image);
                    ...
                    <div className="grid gap-8 pb-10 md:grid-cols-[1.05fr_1fr] md:gap-12 md:pb-14">
                        {image && (
                            <motion.div
                                ...
                                className="relative h-[280px] overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/5 md:h-[360px]"
                            >
                                <Image
                                    src={image}
                                    alt={title}
...
function BrandIntroSection({ contents, language }: BrandIntroSectionProps) {
    ...
    const brandEyebrow = getText(
        getContent<LocalizedText>(contents, "brandEyebrow", {}),
        language
    );

    const brandTitleLine1 = getText(
        getContent<LocalizedText>(contents, "brandTitleLine1", {}),
        language
    );

    const brandTitleHighlight = getText(
        getContent<LocalizedText>(contents, "brandTitleHighlight", {}),
        language
    );

    const brandTitleLine2 = getText(
        getContent<LocalizedText>(contents, "brandTitleLine2", {}),
        language
    );

    const brandParagraph = getText(
        getContent<LocalizedText>(contents, "brandParagraph", {}),
        language
    );

    const brandTags = getTextArray(
        getContent<LocalizedArray>(contents, "brandTags", {}),
        language
    );

    const stats = getContent<StatItem[]>(contents, "stats", []);

    const brandEyebrowLength = brandEyebrow.length;

    const brandEyebrowTextClass =
        brandEyebrowLength > 34
            ? "text-[9px] sm:text-[10px] md:text-[11px] tracking-[0.16em] sm:tracking-[0.18em]"
            : brandEyebrowLength > 24
                ? "text-[10px] sm:text-[11px] md:text-xs tracking-[0.18em] sm:tracking-[0.21em]"
        ...
        >
            ...
            <div className="mx-auto max-w-7xl px-4 md:px-8 xl:px-12">
                <div className="grid gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:gap-16">
                    ...
                    >
                        {brandEyebrow && (
                            <motion.div style={{ y: badgeY }} className="mb-6 inline-flex items-center gap-3">
                                <span className="h-2.5 w-2.5 rounded-full bg-[#02acfa]" />
                                <span
                                    className={`${brandEyebrowTextClass} max-w-[260px] font-semibold uppercase leading-relaxed text-[#0D0D0D]/70 sm:max-w-[360px] md:max-w-[460px]`}
                                >
                                    {brandEyebrow}
                                </span>
                            </motion.div>
                        ...
                        <h2 className="max-w-4xl text-4xl font-extrabold leading-[1.02] tracking-[-0.05em] text-[#0D0D0D] sm:text-5xl md:text-6xl xl:text-[5.1rem]">
                            {brandTitleLine1}
                            <span className="block text-[#02acfa]">{brandTitleHighlight}</span>
                            {brandTitleLine2}
                        </h2>
                    ...
                    {stats.length > 0 && (
                        <motion.div
                            key={`stats-${language}-${stats.map((stat) => stat.value).join("-")}`}
                        ...
                        >
                            {stats.map((stat, index) => {
                                const label = getText(stat.label, language);
                                const description = getText(stat.description, language);

                                if (!stat.value || !label) return null;
...
export default function CompanyPage() {
    ...
    useEffect(() => {
        ...
        const fetchCompany = async () => {
            try {
                const response = await fetch(`${API_BASE}/api/pages/CompanyPage`);

                if (!response.ok) {
                    throw new Error(`Company verisi alınamadı. Status: ${response.status}`);
                }

                const data = (await response.json()) as CompanyPageResponse;
                ...
                const companySection =
                    data.sections?.find((section) => {
                        ...
                        return (
                            sectionName === "company" ||
                            sectionName === "companypage" ||
                            sectionName === "company-page"
                        );
                    }) || data.sections?.[0];

                if (isMounted && companySection?.contents) {
                    setContents(companySection.contents);
                }
            } catch (error) {
    ...
    const servicesEyebrow = getText(
        getContent<LocalizedText>(contents, "servicesEyebrow", {}),
        language
    );

    const servicesTitle = getText(
        getContent<LocalizedText>(contents, "servicesTitle", {}),
        language
    );

    const servicesParagraph = getText(
        getContent<LocalizedText>(contents, "servicesParagraph", {}),
        language
    );

    const services = getContent<ServiceItem[]>(contents, "services", []);

    const visionEyebrow = getText(
        getContent<LocalizedText>(contents, "visionEyebrow", {}),
        language
    );

    const visionTitle = getText(
        getContent<LocalizedText>(contents, "visionTitle", {}),
        language
    );

    const visionParagraph = getText(
        getContent<LocalizedText>(contents, "visionParagraph", {}),
        language
    );

    const visionLabel = getText(
        getContent<LocalizedText>(contents, "visionLabel", {}),
        language
    );

    const visionDiscoverText = getText(
        getContent<LocalizedText>(contents, "visionDiscoverText", {}),
        language
    );

    const visionCards = getContent<VisionItem[]>(contents, "visionCards", []);

    // BURAYI DEĞİŞTİREBİLİRSİN
    if (isLoading) {
        return (
            <main
                className="min-h-screen overflow-hidden text-[#0D0D0D]"
                style={{ backgroundColor: LIGHT_BG }}
            >
                <section className="flex min-h-screen items-center justify-center px-4">
                    <div className="flex flex-col items-center gap-5">
                        <div className="h-11 w-11 animate-spin rounded-full border-2 border-[#02acfa]/25 border-t-[#02acfa]" />

                        <p className="text-sm font-medium tracking-[0.22em] text-[#0D0D0D]/45 uppercase">
                            Artech Software
                        </p>
                    </div>
                </section>
    ...
    return (
        <div
            ...
            style={{ backgroundColor: LIGHT_BG }}
        >
            <BrandIntroSection contents={contents} language={language} />
                    ...
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
                        {visionCards.map((item, index) => (
                            <VisionMissionCard
                                ...
                                language={language}
                                label={visionLabel}
                                discoverText={visionDiscoverText}
                            />
                        ))}
                    </div>
                </div>
