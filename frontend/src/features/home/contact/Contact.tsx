...
type ContactContentValue = string | string[] | LocalizedText | Partial<Record<LanguageCode, string[]>>;
...
function getTextArray(
  value: Partial<Record<LanguageCode, string[]>> | string[] | undefined,
  language: string
): string[] {
  if (!value) return [];
...
export default function Contact() {
  ...
  useEffect(() => {
    const fetchContactCta = async () => {
      try {
        const response = await fetch(`${API_BASE}/api/pages/ContactCTA`, {
          cache: "no-store",
        });

        if (!response.ok) {
          throw new Error(
            `ContactCTA verisi alınamadı. Status: ${response.status}`
          );
        }
        ...
        const contactSection = data.sections?.find(
          (section) => section.name?.toLowerCase() === "contact-cta"
        );

        if (contactSection?.contents) {
          setContents(contactSection.contents);
        }
      } catch (error) {
        console.error("ContactCTA verisi alınamadı:", error);
      }
    };
  ...
  const backgroundImage = getPublicMediaUrl(
  ...
  const examples = getTextArray(
    getContent<Partial<Record<LanguageCode, string[]>>>(
      contents,
      "examples",
      {}
    ),
  ...
  const examplePrefix = getText(
    getContent<LocalizedText>(contents, "examplePrefix", {}),
    language
  );
...
function Typewrite({ examples, examplePrefix }: TypewriteProps) {
  ...
  return (
    <p className="mb-2.5 text-[10px] sm:text-xs md:text-base lg:text-lg font-mono uppercase text-white whitespace-normal md:whitespace-nowrap">
      {examplePrefix && <span className="mr-2">{examplePrefix}</span>}
      {displayText}
