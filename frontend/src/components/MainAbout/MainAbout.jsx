...
const MainAbout = () => {
  ...
  useEffect(() => {
    const fetchMainAbout = async () => {
      try {
        const data = await fetchJson(`${import.meta.env.VITE_API_URL}/api/pages/MainAbout`);
        if (!data) return;

        console.log("MainAbout gelen veri:", data);

        const mainAboutSection = data.sections?.find(
          (section) => section.name?.toLowerCase() === "mainabout"
        );

        if (mainAboutSection) {
          setContents(mainAboutSection.contents || []);
        }
      } catch (error) {
        console.error("MainAbout verisi alınamadı:", error);
      }
    };
